...
//This file contains European Option pricing using binomial model for the Amazon Case
...
double up_factor, uptick_prob, downtick_prob, notick_prob, R, R2, D, risk_free_rate, volatility, div_rate;
...
double option_price_put_black_scholes(const double& S,      // spot price
                                      ...
                                      const double& time,
                                      const double& div)   // time to maturity
{
    double time_sqrt = sqrt(time);
    double d1 = ((log(S/K)+(r-div+(0.5*pow(sigma,2)))*time))/(sigma*time_sqrt);
    double d2 = d1-(sigma*time_sqrt);
    return K*exp(-r*time)*N(-d2) - S*exp(-div*time)*N(-d1);
};
                                       ...
                                       const double& div)   // time to maturity
{
    ...
    double d1 = ((log(S/K)+(r-div+(0.5*pow(sigma,2)))*time))/(sigma*time_sqrt);
    double d2 = d1-(sigma*time_sqrt);
    return S*exp(-div*time)*N(d1) - K*exp(-r*time)*N(d2);
};
...
float american_call_option(int k, int i, float current_stock_price) {
    if (k == no_of_divisions){
//        x++;
//        cout<<"X ="<<x<<" --- "<<max(0.0, (current_stock_price - strike_price))<<endl;
...
{
    //  7 7 0.0056 0.47 20.87 17.7257 0.0447
    ...
    R2 = exp((risk_free_rate)*expiration_time/((float) no_of_divisions));
    D = exp((div_rate)*expiration_time/((float) no_of_divisions));

    
    up_factor = exp(volatility*sqrt((expiration_time)/((float) no_of_divisions)));
    uptick_prob = (R2/D - (1/up_factor))/(up_factor-(1/up_factor));
    //    uptick_prob = 0.39077;
    
    //    Calculating option prices
    double call_price = european_call_option(0, 0);
    //    double put_price = european_put_option(0, 0);
    double call_price_BSM = option_price_call_black_scholes(initial_stock_price, strike_price, risk_free_rate, volatility, expiration_time,div_rate);
    //    double put_price_BSM = option_price_put_black_scholes(initial_stock_price, strike_price, risk_free_rate, volatility, expiration_time,div_rate);
...
//    double call_price_american = american_call_option(0, 0,initial_stock_price);
//    cout << "Binomial Price of an American Call Option = " << call_price_american << endl;
    cout << "--------------------------------------" << endl;
    ...
    float N = 34.85+1.087+(0.6214*6);
    double dil = (N)/(N + 0.6214*7);
    cout <<"Diluted Value of Warrants: $"<<dil*call_price<<endl;

    cout <<"Total Value of Warrants: $"<<0.6214*dil*call_price<<" million"<<endl;
