...
float american_asian_call_option(int k, int i, float current_stock_price, float current_average_stock_price) {
    ...
    {
//        cout<<"Avg stock price at "<<k<<","<<i<<" is: "<<current_average_stock_price<<endl;
        return max(0.0, (current_average_stock_price - current_stock_price));
        
    }
    else
    {
//        cout<<"Avg stock price at "<<k<<","<<i<<" is: "<<current_average_stock_price<<endl;
        return max((current_average_stock_price - current_stock_price), (uptick_prob*american_asian_call_option(k+1, i+1, current_stock_price*up_factor, ((k*current_average_stock_price) + (current_stock_price*up_factor))/((float) k+1)) + (1-uptick_prob)*american_asian_call_option(k+1, i-1, current_stock_price/up_factor, ((k*current_average_stock_price) + (current_stock_price/up_factor))/((float) k+1)))/R);
...
float american_asian_put_option(int k, int i, float current_stock_price, float current_average_stock_price) {
    ...
    {
        cout<<"Avg stock price at "<<k<<","<<i<<" is: "<<current_average_stock_price<<endl;
        return max(0.0, (current_stock_price - current_average_stock_price));
    }
    else
    {
        cout<<"Avg stock price at "<<k<<","<<i<<" is: "<<current_average_stock_price<<endl;
        return max((current_stock_price - current_average_stock_price),
